export default defineEventHandler(async (event) => {
    ...
    const params = {
        ...
        country: country,
        limit: query.limit ? String(query.limit) : '6',
        offset: query.offset ? String(query.offset) : '0'
    }
    
    const config = useRuntimeConfig()
    const apiUrl = `https://${country}.${config.public.categoriesApiUrl}/related-products?${new URLSearchParams(params)}`
